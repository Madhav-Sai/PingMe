...
"""
╔══════════════════════════════════════════════════════════════════╗
║              PingMe — Advanced Ping Scanner v3.0.2 by Madhav       ║
...
def show_file_scan_status(rows: list[dict], results: list[dict], source: str) -> None:
    ...
    host_w = min(34, max(12, max(len(str(row.get("host", ""))) for row in rows)))
    ip_w = min(45, max(15, max(len(str(row.get("ip") or "UNRESOLVED")) for row in rows)))
    status_w, method_w, ttl_w, os_w = 13, 18, 5, 18
    widths = [host_w, ip_w, status_w, method_w, ttl_w, os_w]
    line = "  " + C.PURPLE + "+" + "+".join("-"*(w+2) for w in widths) + "+" + C.RESET
    ...
    for row in rows:
        ...
        values = [
            (host, C.WHITE),
            (ip_value, C.CYAN if ip_value != "UNRESOLVED" else C.YELLOW),
            (status, status_color),
            (method, C.YELLOW),
            (ttl, C.WHITE),
        ...
        print("  " + C.PURPLE + "|" + C.RESET + " " + (" " + C.PURPLE + "|" + C.RESET + " ").join(
            f"{color}{value[:width]:<{width}}{C.RESET}" for (value, color), width in zip(values, widths)
        ) + " " + C.PURPLE + "|" + C.RESET)

    print(line)
    summary = (
        f"  {C.GREEN}Reachable: {reachable_count}{C.RESET}  "
        f"{C.RED}No response: {no_response_count}{C.RESET}  "
        f"{C.YELLOW}Unresolved: {unresolved_count}{C.RESET}"
    )
    if not_scanned_count:
        summary += f"  {C.DIM}Not scanned/excluded: {not_scanned_count}{C.RESET}"
    print(summary + "\n")
...
def read_target_file(path: str) -> tuple[list[str], list[dict]]:
    ...
    if not targets:
        ...
        show_file_scan_status(mappings, [], path)
...
def main():
    ...
    if args.scan:
        ...
        if args.file and file_mappings:
            show_file_scan_status(file_mappings, results, args.file)
