...
def add_windows_user_path(directory: Path) -> None:
    try:
        ...
        current_dir = os.path.normcase(os.path.normpath(str(directory)))

        if current_dir not in normalized:
            entries.append(str(directory))
            winreg.SetValueEx(key, "Path", 0, value_type, ";".join(entries))
            success(f"Added to user PATH: {directory}")
            warn("Open a new terminal before running pingme.")
...
def powershell_completion() -> str:
    all_words = OPTIONS + PING_TOOLS + OUTPUT_FORMATS
